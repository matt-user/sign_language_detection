...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        lstm_out, (hidden, cell) = self.lstm3(lstm_out)
...
def load_and_validate_data():
    ...
    for action in actions:
        ...
        data_stats[action] = {
            ...
            'mean': np.mean([np.mean(seq) for seq in action_sequences]),
            'std': np.mean([np.std(seq) for seq in action_sequences])
        }
        
        print(f"  Loaded {len(action_sequences)} valid sequences")
        print(f"  Data stats - mean: {data_stats[action]['mean']:.4f}, std: {data_stats[action]['std']:.4f}")
...
def train_model_with_validation():
    ...
    # Load data
    X, y, actions, data_stats = load_and_validate_data()
    ...
    # Load best model
    checkpoint = torch.load("best_sign_language_model.pth", weights_only=False)
    ...
    print(classification_report(train_targets, train_predictions, target_names=actions))
...
if __name__ == "__main__":
    model, actions = train_model_with_validation()
    print("\nTraining completed! Model saved as 'best_sign_language_model.pth'") 